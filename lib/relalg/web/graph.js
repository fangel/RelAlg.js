...
define(['../tree', '../util/htmlify', 'd3'], function(Tree, htmlify, d3) {
  ...
  function _graph(context, item) {
    var bb, Lbb, lblOffset, childWidth
    switch (true) {
      case item instanceof Tree.Assignment:
      ...
      case item instanceof Tree.Selection:
        ...
        Lbb = boundingBox(context, item, false)
        var childSize = boundingBox(context, item.relation)
        childOffset = (Math.max(bb.width, Lbb.width) - childSize.width) / 2
        lblOffset = (Math.max(bb.width, Lbb.width) - Lbb.width) / 2
        label(context.current, item, {x: lblOffset, y: -5})
        context.current.append('line').attr({
          x1: bb.width / 2,
          y1: Lbb.height - 15,
          x2: bb.width / 2,
          y2: Lbb.height + layerSpacing - 15,
          stroke: 'black'
        })
        context.current = context.current.append('g').attr('transform', 'translate(' + childOffset + ', ' + (Lbb.height + layerSpacing) + ')')
        _graph(context, item.relation)
        break
      case item instanceof Tree.Union:
      case item instanceof Tree.NaturalJoin:
      ...
      case item instanceof Tree.Join:
        ...
        var leftSize = boundingBox(context, item.left)
          ...
          , combinedChildWidth = leftSize.width + groupSpacing + rightSize.width
          , lblOffset = (Math.max(bb.width, Lbb.width) - Lbb.width) / 2
          , leftOffset = (Math.max(bb.width, combinedChildWidth) - combinedChildWidth) / 2
          , rightOffset = leftOffset + leftSize.width + groupSpacing

        label(context.current, item, {x: lblOffset, y: 1})
        
        var origContext = context.current
        context.current.append('line').attr({
          x1: lblOffset + (Lbb.width / 2) - 5,
          y1: Lbb.height - 10,
          x2: leftOffset + (leftSize.width / 2),
          y2: Lbb.height + layerSpacing - 15,
          stroke: 'black'
        })
        context.current = context.current.append('g').attr('transform', 'translate(' + leftOffset + ', ' + (Lbb.height + layerSpacing) + ')')
        ...
        context.current = context.current.append('g').attr('transform', 'translate(' + rightOffset + ', ' + (Lbb.height + layerSpacing) + ')')
