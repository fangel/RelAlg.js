        ...
        if( criteria.left  instanceof Tree.Attribute 
         && criteria.right instanceof Tree.Attribute )
          if( criteria.left.name == criteria.right.name 
           && criteria.op == '==' ) // Criteria is an equality between two attributes of same name
  ...
  return function( left, criteria, right ) {
    ...
    for( var i in left.data )
      for( var j in right.data )
        if( evaluateRow( criteria, left.header, left.data[i], right.header, right.data[j] ) )
          relation.push( combineRows( left.data[i], right.data[j], blacklist ) );
