 ...
 */
define(['tree', 'util/htmlify', 'vendor/d3'], function(Tree, htmlify, d3) {
  var layerSpacing = 30
    , groupSpacing = 40
  ...
  function boundingBox(context, item, recurse) {
    ...
    switch (true) {
      case item instanceof Tree.RelationReference:
        var lbl = label(context.staging, item)
          , bb = lbl.node().getBBox()
        lbl.remove()
        return bb
      case item instanceof Tree.Projection:
  ...
  function _graph(context, item) {
    switch (true) {
      ...
      case item instanceof Tree.Projection:
			case item instanceof Tree.Rename:
			case item instanceof Tree.Selection:
			  var bb = boundingBox(context, item)
			    , size = boundingBox(context, item, false)
			    , childSize = boundingBox(context, item.relation)
