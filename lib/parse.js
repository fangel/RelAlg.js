define(['parser', 'tree', 'relation'], function(Parser, Tree, Relation) {
  ...
  ParseError.prototype.getPosition = function() {
    var startRow = this.position.first_line
      ...
      , startCol = (this.position.last_column === 0) ? 0 : this.position.last_column + 1
  ...
  ParseError.prototype.showPosition = function() {
    ...
    if (this.token === null) {
      // Tokenization fail
      pre = (this.position.first_column === 0) ? '' : new Array(this.position.first_column + 1 ).join(" ")
      dur = ''
      ...
      if (this.token === 'EOF') {
        ...
        pre = (this.position.last_column === 0) ? '' : new Array(this.position.last_column + 2 ).join(" ")
        dur = ''
